...
class World:
    ...
    def generate_random_building(self) -> None:
        ...
        """
        position = np.random.uniform([0, 0, 0], 
                                   [self.dimensions[0]*0.8, 
                                    self.dimensions[1]*0.8, 0])
    ...
    def update_coverage(self, drone_positions: List[npt.NDArray[np.float64]], 
                       coverage_radius: float = 5.0) -> None:
        """
                    ...
                    for dz in range(-radius_cells, radius_cells + 1):
                        if dx*dx + dy*dy + dz*dz <= radius_cells*radius_cells:
                            ...
                            if (0 <= nx < self.coverage_grid.shape[0] and
                                0 <= ny < self.coverage_grid.shape[1] and
                                0 <= nz < self.coverage_grid.shape[2]):
    ...
    def get_drone_neighbors(self, drone_id: int, 
    ...
    def step(self, dt: float) -> None:
        ...
        drone_positions = [drone.state.position for drone in self.drones 
    ...
    def check_drone_collisions(self, safe_distance: float = 2.0) -> None:
        ...
        for i, drone1 in enumerate(self.drones):
            ...
            for j, drone2 in enumerate(self.drones[i+1:], i+1):
                ...
                distance = np.linalg.norm(
                    drone1.state.position - drone2.state.position
                )
