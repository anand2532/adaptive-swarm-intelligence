...
class Building(Obstacle):
    ...
    def __init__(
        self, position: npt.NDArray[np.float64], size: npt.NDArray[np.float64]
    ):
        """
    ...
    def intersects(self, other: Obstacle) -> bool:
        ...
        return not (
            np.any(self.max_point < other_min) or np.any(self.min_point > other_max)
        )
