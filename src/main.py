...
def parse_args():
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Run with visualization"
    )
    parser.add_argument(
...
def train(config, output_dir, checkpoints_dir, visualize=False):
    ...
    state_dim = (
        15  # position(3) + velocity(3) + obstacle_info(4) + target_info(4) + battery(1)
    )
    ...
    for episode in range(config.training.num_episodes):
        ...
        for step in range(config.training.max_steps):
            ...
            reward = sum(
                controller.compute_reward(drone, world) for drone in world.drones
            )
