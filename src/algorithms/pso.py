...
class PSOOptimizer:
    ...
    def update_global_best(
        self, drones: List[Drone], fitness_values: npt.NDArray[np.float64]
    ) -> None:
    ...
    def calculate_fitness(
        ...
        min_target_distance = min(
            np.linalg.norm(drone.state.position - target) for target in target_points
        )

        # Distance to nearest obstacle
        min_obstacle_distance = min(
            np.linalg.norm(drone.state.position - obstacle.position)
