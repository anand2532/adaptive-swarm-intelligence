...
class MetricsTracker:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "success_rate": sum(ep.mission_success for ep in self.episodes)
            / len(self.episodes),
            ...
            "avg_completion_time": np.mean(
                [ep.completion_time for ep in self.episodes]
            ),
        }
            ...
            "episodes": [
                {
                    ...
                    "mission_success": bool(
                        ep.mission_success
                    ),  # Convert numpy bool to native bool
                    "completion_time": float(ep.completion_time),
                }
                for ep in self.episodes
            ],
            "statistics": {
                k: float(v) for k, v in self.get_statistics().items()
